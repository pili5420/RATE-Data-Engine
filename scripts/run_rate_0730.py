...
def run_rate_0730(snapshot,trading_date,previous_state_id,persist_state=True):
 ...
 for r in snapshot['records']:
  ...
  verified_stage=build_stage_evidence(symbol=r['symbol'],stage_inputs=stage_inputs,previous_stage=stage_inputs['previous_stage'],prior_m7=stage_inputs['prior_m7'],source_state_id=previous_state_id,input_snapshot_id=snapshot['input_snapshot_id'],field_sources=field_sources)
 ...
 h=deterministic_hash(decision); fixture_scope=snapshot.get('provenance',{}).get('source') in ('REPLAY_TEST_ONLY','fixture','fixture-replay'); result={'current_state_id':'rate-state-'+h[:24],'previous_state_id':previous_state_id,'input_snapshot_id':snapshot['input_snapshot_id'],'decision_payload_hash':h,'decision':decision,'top50':top50,'short_top30':short,'long_top30':long,'execution_scope':'FIXTURE_ONLY' if fixture_scope else 'PRODUCTION','07:30_e2e':'PASS_FIXTURE_ONLY' if fixture_scope else 'PASS'}
